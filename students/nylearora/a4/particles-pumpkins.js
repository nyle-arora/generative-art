...
class PumpkinParticleSystem {
    ...
    makePumpkins(){
        this.pumpkinNum = Math.ceil(SLIDER.pumpkinNumber * 5)
        for (var i = 0; i < this.pumpkinNum; i++) {
			let particle = {
				idNumber: i,
	...
	update(p) {
        ...
        let newNum = Math.ceil(SLIDER.pumpkinNumber * 5)
        if (newNum != this.pumpkinNum){
		...
		this.particles.forEach(pt => {
			...
			pt.mouseAttractorForce.mult(SLIDER.pumpkinMouseAttraction**2 * 10)
